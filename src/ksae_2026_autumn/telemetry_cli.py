...
def implementation_hashes():
    files = [
        *sorted((ROOT / "src/ksae_2026_autumn").glob("telemetry*.py")),
        ROOT / "src/ksae_2026_autumn/logging_agent.py",
        ROOT / "tools/run_telemetry.py",
        ROOT / "tools/check_telemetry.py",
        ROOT / "tests/test_telemetry.py",
    ]
...
def run(args):
    ...
    report = check_run(output)
    print(f"Telemetry check: {report['status']} ({output / 'telemetry_check.json'})")
    return 1 if report["status"] == "FAIL" or code != 0 else 0


def self_test(output):
    output = Path(output).expanduser().resolve()
    output.mkdir(parents=True, exist_ok=True)
    suite = unittest.defaultTestLoader.discover(str(ROOT / "tests"), pattern="test_telemetry.py")
    with (output / "self_test.log").open("w", encoding="utf-8") as stream:
    ...
    print((output / "self_test.log").read_text())
...
def main():
    ...
    try:
        ...
        result = check_run(Path(args.directory).expanduser().resolve())
